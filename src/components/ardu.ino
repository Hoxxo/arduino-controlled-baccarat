...
constexpr uint8_t DEALER_WIN_BUTTON = 7;
constexpr uint8_t PLAYER_WIN_BUTTON = 8;
constexpr uint8_t CLEAR_BUTTON = 9;
constexpr uint8_t UNDO_BUTTON = 10;
constexpr uint8_t TIE_WIN_BUTTON = 11;
...
void setup() {
    ...
    pinMode(TIE_WIN_BUTTON, INPUT_PULLUP);
}
...
void loop() {
    ...
    tieButtonState = digitalRead(TIE_WIN_BUTTON) == LOW;
    ...
    if (tieButtonState && !lastTieButtonState) {
        Serial.print('T');
        delay(DEBOUNCE_DELAY);
    }
    ...
    lastUndoButtonState = undoButtonState;
}
